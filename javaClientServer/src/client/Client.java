...
public class Client{
    ...
    private String intToString( int value, int length ){    // returns string of length "length", fulfill with zeros
        ...
        for( int i = 0; i < length - intLength; ++i ){
            zeros.append( "0" );
        }
    ...
    private int getMsg(){
        ...
        switch( header[ 0 ] ){
            case STATEMENT: getStatement( msg ); break;
            case EDIT: getEdit( msg ); break;
            case PUBLIC_KEY: getServerPublicKey( msg ); break;
            case CLIENT_ID: getClientId( msg ); break;
            case FILES: getFileNames( msg ); break;
            default: { return -1; }
        }
        return 0;
    }
    ...
    private void getStatement( Message statement){
        ...
        switch( statement.getStatement() ){
            case KEEP_ALIVE: isAlive = true; break;
            case REQUEST_LOGIN: canLogIn = true; break;
            case LOGIN_ACCEPTED: logged = true; break;
            case LOGIN_REJECTED: logged = false; break;
            case WORK_END: notepad.displayError( "503\nSerwajs anaweilebeilebeilebul.\nServer disconnected you." ); break;
...
//        }
        ...
        writeStatement( REQUEST_LOGIN );
        writeStatement( LOGIN_ACCEPTED );
        return 0;
    }
    ...
    public void createThreads(){
        ...
        createWriter();
        createReader();
        //createKeepAlive();        //TODO commented when using my server
    }

    public void startThreads(){
        writer.start();
        reader.start();
        //keepAlive.start();    //TODO commented when using my server
    }
